...
// feature). Loads a GLB exactly the way CarModel.jsx does — bakes the registry
// rotation, then auto-fits (scale so max dim = 3.4 units, ground to y=0) — and
// reports every mesh's material + bounding box IN FITTED CAR SPACE (the same
// coordinate frame lib/systems.js hotspots use: +X nose, +Y up, ±Z sides).
//
// With a part-map (JSON: { partId: ["materialSubstr", ...] }) it also groups
// meshes into logical parts and prints each part's fitted centroid (the anchor),
// size, mesh count, and matched materials — plus any materials left unmatched, so
// nothing is silently dropped. Used to author data/breakdowns/<slug>.json.
//
...
loader.load(q.get('model'), (g) => {
  const o = g.scene; o.rotation.set(0, Math.PI/2, 0); o.updateMatrixWorld(true);
  ...
  const px = -center.x*s, py = -full.min.y*s, pz = -center.z*s; // CarModel fit position
  const fit = (v) => [ +(v.x*s+px).toFixed(3), +(v.y*s+py).toFixed(3), +(v.z*s+pz).toFixed(3) ];
  const meshes = [];
  o.traverse((n) => {
...
await page.goto(`http://localhost:${port}/_meshpage?model=${encodeURIComponent(glbPath)}`);
...
const result = { scale: out.scale, meshCount: out.meshes.length };
if (partmap) {
  const claimed = new Set();
  result.parts = {};
  for (const [id, pats] of Object.entries(partmap)) {
    const rows = out.meshes.filter((m) => pats.some((p) => m.material.toLowerCase().includes(p.toLowerCase())));
    rows.forEach((r) => claimed.add(r.material));
    const g = union(rows);
